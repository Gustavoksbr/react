...
import Atv5 from '../atv/Atv5';
import rotasAtv5 from './rotasAtv5';
...
const listaAtv5 = ["FormQuiz_", "FormTicket_", "Accordion_", "Messenger_"]
    ...
    return (
        ...
        <BrowserRouter>
            <Routes>
                ...
                {listaAtv5.map((elemento) => {
                    const Componente = rotasAtv5[elemento];
                    if (!Componente) {
                        console.error(`Componente não encontrado: ${elemento}`);
                        return null;
                    }
                    return <Route key={elemento} path={'/Atv5/comp/' + elemento} element={<Componente />} />
                })}
            </Routes>
        </BrowserRouter>
    );
